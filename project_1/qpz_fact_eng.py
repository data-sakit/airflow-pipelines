...
clickhouse_table_name = 'pixbi.FACT_APP'
...
with DAG('fact_app_dag', default_args=default_args, schedule='@daily') as dag:
    ...
    def count_row_ms():
        ...
        cursor = conn.cursor()
        cursor.execute("select COUNT(*) as row_count from dbo.app_internal_order_fact_item app")
...
SELECT
                ...
                division,
                cast(provision_year as Varchar) as Year,
                quantity,
                cost,
                CASE 
                ...
                END as Directory
            FROM dbo.app_internal_order_fact_item app
            LEFT JOIN dbo.contractor c ON app.organization_id = c.id
            LEFT JOIN dbo.measure_unit mm ON app.measure_unit_id = mm.id
        ...
        with Client(host=clickhouse_host, port=clickhouse_port, user=clickhouse_user, password=clickhouse_password) as client:
            client.execute(f'''
                ...
                (
                    ...
                    `construction_name` Nullable(String),
    ...
    def insert_into_clickhouse(df):
        with Client(host=clickhouse_host, port=clickhouse_port, user=clickhouse_user, password=clickhouse_password) as client:
            ...
            client.execute(
                f'INSERT INTO {clickhouse_table_name} (id, contractor_name, site_name, procurement_plan_type, construction_name, enlarged_nomenclature,nomenclature, measure , reserve_document, month_name, Dt,division,Year, quantity, cost, Directory) VALUES',
                data
            )
