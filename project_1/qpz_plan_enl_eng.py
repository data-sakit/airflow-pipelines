...
clickhouse_table_name = 'pixbi.PLAN_APP_ENL'
...
with DAG('plan_app_enl_dag', default_args=default_args, schedule='@daily') as dag:
    ...
    def count_row_ms():
        ...
        cursor = conn.cursor()
        cursor.execute("select COUNT(*) as row_count from dbo.app_enlarged_nomenclature_plan_item app")
    ...
    def execute_sql():
        ...
        SELECT
                ...
                aen.name AS enlarged_nomenclature,
                mu.name AS measure,
                ...
                CASE 
                    ...
                    and cs.name='Прорыв'
                    and procurement_plan_type='Материалы'
                    and (co.name is NUll or co.name in('Прорыв','-',''))
                    and LEFT(division, CHARINDEX(';', division + ';') - 1) = 'Дирекция "Прорыв"'
                    ...
                    and (LEFT(division, CHARINDEX(';', division + ';') - 1) in('Дирекция Курская АЭС','ОП "Дирекция по строительству объектов в Курской области"',''))
	                ...
	                and (LEFT(division, CHARINDEX(';', division + ';') - 1) in('Филиал "Собственное управление строительством"',
									 'Дирекция НИТИ',''))
                    then '3_Дирекция по строительству_2.2_НИТИ'

                    when c.name  ='Акционерное общество "КОНЦЕРН ТИТАН-2"' 
                    and cs.name='Прорыв'
                    and procurement_plan_type ='Материалы и оборудование'
                    and (co.name is NUll or co.name in('Прорыв','-',''))
                    and LEFT(division, CHARINDEX(';', division + ';') - 1) = 'ОП «Сибирь»' 
                    then '4_Дирекция по строительству_2.3_ОП «Сибирь»'
                    
                    when c.name  ='Акционерное общество "КОНЦЕРН ТИТАН-2"' 
                    and cs.name='ЦКП "СКИФ"'
                    and procurement_plan_type in('Материалы')
                    and (co.name is NUll or co.name in('СКИФ','','-'))
                    and (LEFT(division, CHARINDEX(';', division + ';') - 1) in('Дирекция ЦКП «СКИФ»',
                    ...
                    and LEFT(division, CHARINDEX(';', division + ';') - 1) ='Филиал "Собственное управление строительством"'
                    then '6_Дирекция по строительству_2.5_ФДРЦ'	
                    
                    when c.name  ='Акционерное общество "КОНЦЕРН ТИТАН-2"' 
                    and cs.name='ЦКП "СКИФ"'
                    and procurement_plan_type ='Оборудование'
                    and (co.name is NUll or co.name in('СКИФ','','-'))
                    and (LEFT(division, CHARINDEX(';', division + ';') - 1) in('Дирекция ЦКП «СКИФ»','Дирекция собственных сил программы ЦКП "СКИФ" (вспомогательные здания)',''))
                    then '7_ДПО_3.1_СКИФ'	                
                    ...
                    and (LEFT(division, CHARINDEX(';', division + ';') - 1) in('Дирекция по поставкам оборудования',
                    ...
                    and (LEFT(division, CHARINDEX(';', division + ';') - 1) in('Утилизация Марадыковский',''))
                    ...
                    and (LEFT(division, CHARINDEX(';', division + ';') - 1) in('Дирекция "Прорыв"',''))
                    ...
                    and procurement_plan_type ='Оборудование'
                    and (co.name is NUll or co.name in('НИТИ','','-'))
                    and (LEFT(division, CHARINDEX(';', division + ';') - 1) in ('Дирекция НИТИ',''))
                    ...
                    and procurement_plan_type ='Материалы и оборудование'
                     and (co.name is NUll or co.name in('ФДРЦ','','-'))
                    and LEFT(division, CHARINDEX(';', division + ';') - 1) = 'Дирекция ФДРЦ'
                    ...
                    and LEFT(division, CHARINDEX(';', division + ';') - 1) = 'Дирекция СЕНЕЖ'
                    ...
                    and (LEFT(division, CHARINDEX(';', division + ';') - 1) in('Дирекция НМИЦ Вишневского',''))
                    ...
                    and LEFT(division, CHARINDEX(';', division + ';') - 1) = 'Дирекция Норильск'
                    ...
                    and procurement_plan_type ='Материалы и оборудование'
                    and co.name  in('Утилизация (Горный)','Утилизация (Щучье)')
                    and LEFT(division, CHARINDEX(';', division + ';') - 1) = 'Дирекция по строительству объектов ПТК'
                    ...
                    and cs.name='ЛАЭС-2'
                    and procurement_plan_type ='Материалы и оборудование'
                    and LEFT(division, CHARINDEX(';', division + ';') - 1) !='СЭМ, МЗУ'
                    ...
                    and cs.name='ЛАЭС-2'
                    and procurement_plan_type in('Материалы и оборудование','Оборудование','Материалы')
            ...
            from dbo.app_enlarged_nomenclature_plan_item app 
            ...
            left join dbo.app_enlarged_nomenclature aen on aen.id = app.nomenclature_id 
            left join dbo.measure_unit mu on mu.id = app.measure_unit_id 
            left join dbo.[month] m on m.id = app.month_id
            left join dbo.period p on p.id = app.period_id
        """
        df = pd.read_sql(query, conn)
        conn.close()
        column_names = ["contractor_name", "site_name", "Dt", "procurement_plan_type", "construction_name", "enlarged_nomenclature", "measure","month_name","division"]
        df['Year'] = df['Year'].str.replace(" г.", "", regex=False).astype(str)   
    ...
    def insert_into_clickhouse(df):
        with Client(host=clickhouse_host, port=clickhouse_port, user=clickhouse_user, password=clickhouse_password) as client:
            ...
            client.execute(
                f'INSERT INTO {clickhouse_table_name} (id, contractor_name, site_name, Dt, procurement_plan_type, construction_name, enlarged_nomenclature, measure, month_name,division,Year, quantity, cost, Directory) VALUES',
                data
            )
