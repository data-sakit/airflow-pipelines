...
to_emails_ЛАЭС_2=['t.chervyakova@titan2.ru','d.shilovskiy@titan2.ru','k.lyalyuga-bizin@titan2.ru','k.openko@titan2.ru','i.elnikova@titan2.ru','s.mamedov@titan2.ru']


with DAG('solut_parsek_email_laes', default_args=default_args, schedule='30 8 * * *',catchup=False) as dag:
    ...
    @task(task_id="vis1_laes")
    def vis1_laes(df):
        yesterday=(datetime.now()-timedelta(days=1)).date()
        df_yes=df[(df['dt']==yesterday)&(df['project']=='ЛАЭС-2')]
        ...
        buffer.seek(0)
        plt.close()
        vis_1_laes = base64.b64encode(buffer.getvalue()).decode('utf-8')
        return vis_1_laes

    @task(task_id="vis2_laes")
    def vis2_laes(df):
        yesterday = (datetime.now() - timedelta(days=1)).date()
        df_yes=df[(df['dt']==yesterday)&(df['project']=='ЛАЭС-2')]
        ...
        buffer.seek(0)
        plt.close()
        vis_2_laes = base64.b64encode(buffer.getvalue()).decode('utf-8')
        return vis_2_laes

    @task(task_id="vis3_laes")
    def vis3_laes(df):
        yesterday = (datetime.now() - timedelta(days=1)).date()
        df_yes=df[(df['dt']==yesterday)&(df['project']=='ЛАЭС-2')]
        ...
        buffer.seek(0)
        plt.close()
        vis_3_laes = base64.b64encode(buffer.getvalue()).decode('utf-8')
        return vis_3_laes

    @task(task_id="vis4_laes")
    def vis4_laes(df):
        yesterday = (datetime.now() - timedelta(days=1)).date()
        df_yes=df[(df['dt']<=yesterday)&(df['project']=='ЛАЭС-2')]
        ...
        buffer.seek(0)
        plt.close()
        vis_4_laes = base64.b64encode(buffer.getvalue()).decode('utf-8')
        return vis_4_laes

    @task(task_id="vis5_laes")
    def vis5_laes(df):
        yesterday = (datetime.now() - timedelta(days=1)).date()
        df_yes=df[(df['dt']<=yesterday)&(df['project']=='ЛАЭС-2')]
        ...
        buffer.seek(0)
        plt.close()
        vis_5_laes = base64.b64encode(buffer.getvalue()).decode('utf-8')
        return vis_5_laes

    @task(task_id="vis6_laes")
    def vis6_laes(df):
        yesterday = (datetime.now() - timedelta(days=1)).date()
        activity_cols = [
        'Break_activity_duration',
        ...
        buffer.seek(0)
        plt.close()
        vis_6_laes = base64.b64encode(buffer.getvalue()).decode('utf-8')
        return vis_6_laes

    @task(task_id="vis7_laes")
    def vis7_laes(df):
        yesterday = (datetime.now() - timedelta(days=1)).date()
        activity_cols = [
        'Break_activity_duration',
        ...
        buffer.seek(0)
        plt.close()
        vis_7_laes = base64.b64encode(buffer.getvalue()).decode('utf-8')
        return vis_7_laes

    @task(task_id="send_email_laes")
    def send_email_laes(vis_1_laes,vis_2_laes,vis_3_laes,vis_4_laes,vis_5_laes,vis_6_laes,vis_7_laes):
        ...
        decoded_images = []
        for i, vis in enumerate([vis_1_laes, vis_2_laes, vis_3_laes, vis_4_laes, vis_5_laes, vis_6_laes, vis_7_laes], start=1):
        ...
        pdf_filename = "/opt/airflow/dags/Solut/ЛАЭС_2.pdf"  # Укажите нужный путь
        cover = "/opt/airflow/dags/Solut/ЛАЭС_2.jpg"
        ...
        visuals = [("Визуал 1", vis_1_laes), ("Визуал 2", vis_2_laes), ("Визуал 3", vis_3_laes),
                ("Визуал 4", vis_4_laes), ("Визуал 5", vis_5_laes), ("Визуал 6", vis_6_laes), ("Визуал 7", vis_7_laes)]
        ...
        <html>
            <body>
                <p>Нажмите <a href="https://pixbi.titan2.ru/shared/view/3467e084-f681-4ba0-9a53-5ed790f4a5ba?ticket=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJodHRwOi8vc2NoZW1hcy54bWxzb2FwLm9yZy93cy8yMDA1LzA1L2lkZW50aXR5L2NsYWltcy9uYW1lIjoiVGlja2V0VXNlciIsIm5iZiI6MTc1Mzg3NDI0NiwiZXhwIjoyMDY5NDA3MDQ2fQ.1AZMTrOA05Dtu-mFKtrYf4x5MxVvYypRImdGJkiXxRk">здесь</a>, чтобы посмотреть отчет.</p>
                {visuals_html}
            </body>
        </html>
        """
        ...
        email_task = EmailOperator(
            task_id='send_email_laes_task',
            to=to_emails_ЛАЭС_2,
            subject='Ежедневый отчет - ЛАЭС-2',
            html_content=html_content,
            files=[pdf_filename]  # Передаем только путь к файлу
        )
        email_task.execute(context={})

    vis_1_laes = vis1_laes(d)
    vis_2_laes = vis2_laes(d)
    vis_3_laes = vis3_laes(d)
    vis_4_laes = vis4_laes(d)
    vis_5_laes = vis5_laes(d)
    vis_6_laes = vis6_laes(d)
    vis_7_laes = vis7_laes(d)

    send_email_laes(vis_1_laes,vis_2_laes,vis_3_laes,vis_4_laes,vis_5_laes,vis_6_laes,vis_7_laes)
